...
**为什么需要专编组件**：
- 番茄小说网不提供官方 API（作者后台为前端交互式页面）
- 无法通过简单的 HTTP 请求模拟（涉及 JavaScript 渲染、动态表单、浏览器 Cookie 管理等）
- 需要完整的浏览器自动化方案

**架构设计**：

1. **Cookie 管理**
   - 用户从番茄小说网复制 Cookie 字符串
   - Go Handler 接收、AES 加密后存入 PostgreSQL（fanqie_accounts 表）
   - Python sidecar 按需查询解密后的 Cookie，注入到 Playwright 浏览器上下文

2. **浏览器自动化执行**
   - Playwright Chromium 在 Python sidecar 中 headless 运行
   - 导航到作者后台、自动填充章节、提交表单、等待确认
   - 间隔 3 秒/章避免平台速率限制
   - 自动截图作为操作验证（上传页面、成功确认等）

3. **持久化与断点续传**
   - 上传状态记录到 PostgreSQL（fanqie_uploads 表）
   - 按章记录：status（pending/uploading/success/failed）、retry_count、last_error
   - 允许用户选择重新上传失败的章节

4. **系统集成**
   - Go Handler（handler_fanqie.go）作为反向代理，接收前端请求
   - 参数验证、权限检查、账号解密委托 Go 完成
   - 代理 HTTP 请求到 Python sidecar（/fanqie/upload 端点）
   - 通过 Redis TaskQueue 支持单章/批量异步上传

**技术栈**：
- `Playwright Python SDK`（浏览器自动化）
- `asyncio`（并发上传控制）
- `DOM 选择器`（元素定位）
...
1. **PostgreSQL 故障**
   - 其他服务无法启动（所有业务依赖关系数据）
   - 自动回滚机制：每个事务失败自动 rollback，不产生脏数据

2. **Neo4j 故障**
   - 图谱查询返回空结果
   - Agent planner 降级为启发式 fallback（不依赖知识图谱）
   - 记忆功能降级，但不影响生成

3. **Qdrant 故障**
   - 向量检索返回空结果或异常
   - Agent 依然可以运行（Lost-in-Middle 排布降级为仅使用 Redis 短期记忆）

4. **Redis 故障**
   - Task Queue 无法使用（无法异步任务）
   - Session 过期（用户需要重新登录）
   - 短期记忆丢失（降级为无滑窗，每章孤立生成）

5. **Python sidecar 故障**
   - Agent 生成、参考书分析、审计功能不可用
   - Go 继续提供项目/章节/角色的基本 CRUD
   - Task Queue 可恢复（重启后 retry）
