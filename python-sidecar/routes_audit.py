...
ai_pattern_detection必须覆盖中文网文AI指纹：看了一眼/心跳漏了一拍/眼眶红了三件套；不是……是……模板；沉默/安静了X秒；空气中弥漫着XX气味；空调/显示器/机器嗡鸣；咖啡/星巴克/排骨/烤肉/日料默认细节偏置；过度积极而缺少真实负面情绪。
禁止输出任何JSON以外的内容。"""
...
_AI_FLAVOR_WORDS = [
    ...
    "看了一眼","心跳漏了一拍","眼眶红了","愣了一下","空气中弥漫着",
    "嗡鸣声","心中暗道","眼中闪过一丝","一股","嘴角勾起",
]
...
def _heuristic_audit(text: str, context: dict) -> dict:
    ...
    regex_hits = []
    for label, pat in [
        ("不是……是……模板", r"不是[………\.]{1,3}是|不是[^。！？]{0,12}而是"),
        ("机械沉默停顿", r"(?:沉默|安静)[了得]?[\d一二三四五六七八九十半两]*秒"),
        ("环境嗡鸣套话", r"(?:空调|显示器|机械|冰箱|机器).{0,12}(?:嗡鸣|嗡嗡)"),
        ("默认生活细节", r"咖啡|星巴克|糖醋排骨|炖排骨|烤肉店|日料店"),
    ]:
        count = len(re.findall(pat, text))
        if count >= (4 if label == "默认生活细节" else 1):
            regex_hits.append(f"{label}×{count}")
    all_ai_hits = ai_hits + regex_hits
    ai_score = max(0.0, 1.0 - len(all_ai_hits) * 0.08)
    dimensions["ai_pattern_detection"] = {
        "score": ai_score,
        "passed": len(all_ai_hits) < 5,
        "issues": [f"检测到AI味高频词/指纹：{', '.join(all_ai_hits[:8])}"] if all_ai_hits else [],
    }

    # high_freq_ai_words
    dimensions["high_freq_ai_words"] = {
        "score": ai_score,
        "passed": len(all_ai_hits) < 3,
        "issues": [f"高频AI惯用词/指纹（{len(all_ai_hits)}处）"] if len(all_ai_hits) >= 3 else [],
    }
